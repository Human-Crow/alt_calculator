...
function factorMap(value: number, gen2: number): VariantMap {
    return new Map([
...
const tier_factors = new Map<number, VariantMap>([
    [1, factorMap(1  ,  1)],
    [2, factorMap(1.5,  2)],
    [3, factorMap(2  ,  4)],
    [4, factorMap(3  ,  8)],
...
function get_tier_factor(
    tiers: BuildMap,
    building: BuildingId, 
    variant: VariantId = V.STD
): number {
...
export function get_belt_amount(tiers: BuildMap, items: number): number {
    const speed = tiers.get(B.Belt) ?? 0;
    return items / speed;
}


export function get_build_amount(
    settings: Settings,
    item_name: ItemId, 
    variant: VariantId, 
    amount: number,
