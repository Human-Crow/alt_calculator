    ...
    if (alt_ratio > 0 && alt_ratio < 1) {
        ...
        for (const [ratio, variant] of entries) {
            ...
            for (const item of recipe.materials) {
                ...
                build_tree(
                    specific_amount,
                    item.name, alt_ratios, gen, split_nodes
                )
            }
        }
        return {
...
function build_split_branches(
    alt_ratios: ItemMap,
    gen: VariantId,
    split_nodes: SplitMap, 
): RecipeNode[] {
    const branches: RecipeNode[] = [];
    ...
    for (const [item_key, amount] of split_nodes) {
        ...
        const nodes = recipe.materials.flatMap((item) => {
            ...
            return [
                build_tree(
                    specific_amount,
                    item.name, alt_ratios, gen,
                    new Map<ItemKey, number>()
                )
            ];
        });
        branches.push({
...
export function build_full_tree(settings: Settings): RecipeNode[] {
    const {
        selected_item, goal_amount,
        coal_pp, nuclear_pp, alt_ratios, gen
    } = settings;
