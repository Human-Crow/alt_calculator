...
const default_values = new Map<string, string>();


export function get_default(html_id: string) {
...
export function get_elem_value(el: HTMLElement | string) {
    if (typeof el === "string") {
        const real_el = document.getElementById(el);
        if (!real_el) throw new Error(`Element '${el}' does not exist!`);
        el = real_el;
    }
