...
export function create_num_el(
    is_rounded: boolean,
    amount: number, 
    f_formatter: (x: number) => string, 
    r_formatter: (x: number) => number | string,
    wrapper_tag?: string,
    color?: string
): HTMLElement {
...
function toggle_number_view() {
    ...
    const els: NodeListOf<HTMLElement> = document.querySelectorAll(".number");
    els.forEach(el => {
        el.textContent =
            mode === "full"
                ? (el.dataset.full ?? "")
...
function get_number_el(target: EventTarget | null): HTMLElement | null {
...
export function click_number(event: PointerEvent) {
