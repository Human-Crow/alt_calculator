...
export const html_ids: string[] = Array.from(bulk_map.values());


function normalize_select_value(str: string): string {
    return str
        .trim()
        .toLowerCase()
...
function apply_value(el: HTMLElement, value: string) {
...
type BulkInput = Record<string, string> | string[] | undefined

function parse_input(value: string): BulkInput {
    value = value.trim();
...
function get_bulk_input(value: string): string {
    value = value.trim();
    try {
...
function copy_bulk(button: HTMLButtonElement, text: string) {
